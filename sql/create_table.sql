...
(
    ...
    updateInterval decimal
(
    7,
    2
) default 0.50 null comment '更新间隔，以小时为单位',
    sort int default 0 not null comment ' 排序',
    createTime datetime default CURRENT_TIMESTAMP not null comment '创建时间',
    updateTime datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
    isDelete tinyint default 0 not null comment '是否删除',
...
(
    256
) not null comment '工作年限',
    jobPosition varchar
(
    256
) not null comment '工作岗位',
    difficulty varchar
(
    50
) not null comment '面试难度',
...
(
    20
) not null comment '本地用户id',
    `nickname` varchar
(
    200
) collate utf8mb4_general_ci default null comment '昵称',
    `avatar` varchar
(
    200
) collate utf8mb4_general_ci default null comment '头像',
    `platform` varchar
(
    20
) character set utf8mb4 collate utf8mb4_general_ci not null comment '平台：github/gitee',
    `openid` varchar
(
    100
) character set utf8mb4 collate utf8mb4_general_ci not null comment '平台用户id',
    `access_token` varchar
(
    500
) character set utf8mb4 collate utf8mb4_general_ci default null comment 'access_token',
    `refresh_token` varchar
(
    500
) character set utf8mb4 collate utf8mb4_general_ci default null comment 'refresh_token',
...
(
    ...
    unique key `idx_platform_openid`
(
    `platform`,
    `openid`
)
...
(
    15,
    2
) NOT NULL COMMENT '打赏金额（精度：分）',
    `remark` VARCHAR
(
    512
) DEFAULT NULL COMMENT '转账说明/备注',
    `isDelete` tinyint default 0 not null comment '是否删除',
...
(
    ...
    -- 外键约束
    CONSTRAINT `fk_donor_user` FOREIGN KEY
(
    `userId`
) REFERENCES `user`
(
...
(
    50
) NOT NULL COMMENT '英雄英文标识(如177)',
    cname VARCHAR
(
    50
) NOT NULL COMMENT '中文名(如苍)',
    title VARCHAR
(
    100
) NOT NULL COMMENT '称号(如苍狼末裔)',
...
(
    500
) COMMENT '皮肤列表(用|分隔，如苍狼末裔|维京掠夺者|苍林狼骑)',
    officialLink VARCHAR
(
    255
) COMMENT '官网详情页链接',
...
(
    20
) NOT NULL AUTO_INCREMENT COMMENT '主键ID' PRIMARY KEY,
    email VARCHAR
(
    256
) COMMENT '被封禁的完整邮箱地址',
...
(
    64
) NOT NULL COMMENT '邮箱后缀（如 .com、.net）',
    reason VARCHAR
(
    256
) default '临时邮箱' COMMENT '封禁理由',
    createTime datetime default CURRENT_TIMESTAMP not null comment '创建时间',
    updateTime datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间'
...
(
    50
) not null comment '动作类型：like-点赞、at-@提及、reply-回复、comment-评论、follow-关注、share-分享',
    sourceId bigint not null comment '事件源 ID，如帖子ID、评论ID 等',
    sourceType int null comment '事件源类型：1- 帖子、2- 评论等',
    sourceContent varchar
(
    256
) not null comment '事件源的内容，比如回复的内容，回复的评论等等',
    url varchar
(
    256
) not null comment '事件所发生的地点链接 url',
...
(
    100
) NOT NULL COMMENT '词语名称',
    category VARCHAR
(
    50
) NOT NULL COMMENT '词库分类: undercover-谁是卧底, draw-default-你画我猜默认, draw-hero-你画我猜王者荣耀, draw-idiom-你画我猜成语',
    wordType VARCHAR
(
    50
) COMMENT '词语类型（如：水果、动物、王者英雄、成语等）',
    createTime datetime default CURRENT_TIMESTAMP not null comment '创建时间',
    updateTime datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间'
...
(
    ...
    code        VARCHAR(64)                        NOT NULL UNIQUE COMMENT '模板唯一码，例如 sword_iron_01',
    name        VARCHAR(100)                       NOT NULL COMMENT '物品名称',
    category    VARCHAR(50)                        NOT NULL COMMENT '物品大类：equipment-装备类（能穿戴的）、consumable-消耗品（药水/卷轴/食物）、material-材料（强化石/合成材料）',
    sub_type    VARCHAR(50) COMMENT '物品子类型，例如 weapon 武器、head 头盔、foot 鞋子、hand 手套',
    equip_slot  VARCHAR(50) COMMENT '可穿戴槽位: head-头部, hand-手部, foot-脚部, weapon-武器；NULL 表示无法穿戴',
    rarity      TINYINT                            NOT NULL DEFAULT 1 COMMENT '稀有度等级（1-8，数字越高越稀有）',
    levelReq    INT      DEFAULT 1 COMMENT '使用等级需求',
    baseAttack  INT      DEFAULT 0 COMMENT '基础攻击力',
    baseDefense INT      DEFAULT 0 COMMENT '基础防御力',
    baseHp      INT      DEFAULT 0 COMMENT '基础生命值',
    mainAttr    VARCHAR(512) COMMENT '非常规属性/词缀(JSON)，格式: [{k,v},...]',
    icon        VARCHAR(255) COMMENT '物品图标地址',
    description TEXT COMMENT '物品描述',
    stackable   TINYINT(1) DEFAULT 0 COMMENT '是否可叠加，0-不可叠加，1-可叠加（如消耗品）',
    removePoint INT      DEFAULT 10 COMMENT '分解后获得的积分',
    createTime  datetime default CURRENT_TIMESTAMP not null comment '创建时间',
    updateTime  datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
    isDelete    tinyint  default 0                 not null comment '是否删除，0-正常，1-已删除',
    INDEX (category) COMMENT '物品大类索引',
    INDEX (equip_slot) COMMENT '装备槽位索引',
    INDEX (rarity) COMMENT '稀有度索引'
