...
public class PetStatusUpdateJob {
    ...
    public void updatePetLevel() {
        ...
        if (onlineUserList.isEmpty()) {
            ...
            log.info("开始执行宠物等级更新任务，只要饥饿度或心情值有一个大于0的宠物就可以获得经验");
            ...
            try {
                ...
                if (updatedCount > 0) {
                    log.info("在线用户宠物经验更新成功，共更新{}个宠物（饥饿度或心情值至少有一个大于0）", updatedCount);
                } else {
                    log.info("没有符合条件的宠物需要更新经验（可能是饥饿度和心情值都为0）");
                }
     ...
     * 每小时更新宠物状态
     * 每小时扣除5点饥饿度、3点心情值
     * 注意：饥饿度和心情值为0的宠物无法获得经验，30级宠物不扣减属性值
     */
    @Scheduled(fixedRate = 3600000) // 3600000毫秒 = 1小时
    ...
    public void dailyPetPointsGeneration() {
        ...
        try {
            ...
            if (updatedCount > 0) {
                log.info("宠物每日积分产出成功，共有{}个宠物产出积分（饥饿度或心情值至少有一个大于0）", updatedCount);
            } else {
                log.info("没有符合条件的宠物产出积分（可能是饥饿度和心情值都为0）");
            }
